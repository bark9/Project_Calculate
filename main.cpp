...
private:
  ...
  /**
   * @brief вызываем функцию на  корректность системы счисления
   * @param base Строковое представление системы счисления
   * @return 0 если корректно, 1 если нет
   */
  int validate_base(std::string base) {
    return function_validate_base(base);
  }

  /**
   * @brief вызывает фунцию проверки корректности числа для заданной системы счисления
   * @param number Проверяемое число
   * @param base_str Система счисления в виде строки
   * @return 0 если корректно, 1 если нет
   */
  int validate_number(std::string number, std::string base_str) {
    return function_validate_number(number, base_str);
  }

  /**
   * @brief вызываем функцию Конвертирует десятичное число в шестнадцатеричное
  ...
  std::string decimalTo16(std::string number) {
     return function_decimalTo16(number);
  }

  /**
   * @brief вызываем функцию конвертации десятичное число в двоичную или восьмеричную систему
   * @param number Десятичное число в виде строки
  ...
  std::string decimalTo_2_8(std::string number, std::string base) {
    return function_decimalTo_2_8(number, base);
  }
  ...
  std::string toDecimal(std::string number, std::string base) {
    return function_toDecimal(number, base);
  }
  ...
  void convert() {
    ...
    std::string result = function_convert(number, base, base_target);
